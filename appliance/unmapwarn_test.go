...
// reservation on the volume, and returns the volume and the coordinator.
func stageHolder(t *testing.T, holder string) (*Coordinator, storage.Volume) {
	t.Helper()
	c, v, _ := stageHolderAt(t, holder)
	return c, v
}
...
func stageHolderAt(t *testing.T, holder string) (*Coordinator, storage.Volume, string) {
	t.Helper()
	store, err := storage.Open(t.TempDir())
	if err != nil {
		t.Fatal(err)
	}
	// TWO volumes, and the one under test is deliberately NOT first.
	//
	...
	// object for any volume that did not land on fileio_0 -- and an earlier
	// version of THIS fixture staged its volume at fileio_0, so the test
	// agreed with the bug and passed. Only the live run caught it. Staging
	// off-zero is what makes this test able to fail for the right reason.
	other, err := store.Create(1<<20, 0)
	if err != nil {
		t.Fatal(err)
	}
	...
	c := &Coordinator{
		...
		st: dbState{
			Hosts: []*Host{
				{UUID: "h-holder", IQNs: []string{"iqn.x:holder"}},
				{UUID: "h-other", IQNs: []string{"iqn.x:other"}},
			},
			Attachments: []*Attachment{
				{VolumeUUID: other.UUID, HostUUID: "h-holder", LUN: 0, Desired: "attached"},
				{VolumeUUID: v.UUID, HostUUID: "h-holder", LUN: 1, Desired: "attached"},
				{VolumeUUID: v.UUID, HostUUID: "h-other", LUN: 1, Desired: "attached"},
			},
			// Seeded so the index is deterministic: exportIndex allocates while
			// ranging a map, so leaving it empty makes which volume gets 0 a
			// coin flip -- and a flaky fixture is worse than a weak one.
			Exports: map[string]int{other.UUID: 0, v.UUID: 1},
		},
		// A real db path, so commit() PERSISTS successfully and then fails in
		// reconcile against the unstaged kernel tree -- which is exactly the
		// ordering that made the dropped-warning bug reachable.
...
func TestDetachingTheHolderWarns(t *testing.T) {
	...
	got := c.fenceLossWarning(v.UUID, "h-holder")
	if got == "" {
...
func TestDetachingANonHolderDoesNotWarn(t *testing.T) {
	c, v := stageHolder(t, "iqn.x:holder")
	if got := c.fenceLossWarning(v.UUID, "h-other"); got != "" {
...
func TestNoReservationDoesNotWarn(t *testing.T) {
	c, v := stageHolder(t, "")
	if got := c.fenceLossWarning(v.UUID, "h-holder"); got != "" {
...
func stageResType(t *testing.T, c *Coordinator, v storage.Volume, root, resType string) {
	t.Helper()
	name := backstoreName(v.UUID)
	for _, b := range c.desiredLIO().Backstores {
		if b.Name != name {
...
func TestLastAttachmentSaysTheReservationMayReturn(t *testing.T) {
	...
	withOther := c.fenceLossWarning(v.UUID, "h-holder")
	...
	for _, a := range c.st.Attachments {
		if a.VolumeUUID == v.UUID && a.HostUUID == "h-other" {
			continue
		}
		kept = append(kept, a)
	}
	...
	last := c.fenceLossWarning(v.UUID, "h-holder")
...
func TestUnmapWarningSurvivesACommitFailure(t *testing.T) {
	...
	warning, err := c.Lununmap(v.UUID, "h-holder")
	if err == nil {
...
func TestUnmapRESTBodyCarriesTheWarning(t *testing.T) {
	...
	body, _ := json.Marshal(map[string]string{"host": "h-holder"})
	req := httptest.NewRequest(http.MethodPost, "/volumes/"+v.UUID+"/lununmap",
