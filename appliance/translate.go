...
func (c *Coordinator) desiredLIO() lio.Config {
	...
	for _, a := range c.st.Attachments {
		if a.Desired == "attached" {
			if v, ok := c.store.Get(a.VolumeUUID); ok && v.State == storage.Ready {
				exported[a.VolumeUUID] = true
			}
		}
	}
	...
	for vol := range exported {
		...
		cfg.Backstores = append(cfg.Backstores, lio.Backstore{
			...
			Attributes: map[string]string{
				"block_size": strconv.Itoa(v.BlockSizeOrDefault()),
	...
	for _, h := range c.st.Hosts {
		...
		for _, a := range c.st.Attachments {
			if a.HostUUID == h.UUID && a.Desired == "attached" && exported[a.VolumeUUID] {
				mls = append(mls, lio.MappedLUN{Index: a.LUN, TPGLUN: c.st.Exports[a.VolumeUUID]})
			}
		}
		for _, iqn := range h.IQNs {
